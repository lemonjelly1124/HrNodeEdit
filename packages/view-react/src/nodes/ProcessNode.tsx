...
export const ProcessNode = ({ data, id,selected }: any) => {
    ...
    const onLeftMouseEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const onRightMouseEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const handleSubBtnClick = () => {
        ...
        if (onNodeStateChanged) {
            onNodeStateChanged(id, { isSubThread: true });
        }
    }
    ...
    return (
        <div className={selected ? `node-selected ${data.theme}` : `node ${data.theme}`} onContextMenu={handleContextMenu} >
            ...
            <Handle className="node-handle-s" type="source" position={data.isSwapped ? Position.Left : Position.Right} style={{ top: '50%' }} isConnectable={true} onMouseEnter={onRightMouseEnter} />
            <Handle className="node-handle-t" type="target" position={data.isSwapped ? Position.Right : Position.Left} style={{ top: '50%' }} isConnectable={true} onMouseEnter={onLeftMouseEnter} />
            {isVisible && (
